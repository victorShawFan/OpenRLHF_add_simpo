...
class DPOTrainer(ABC):
    ...
    def __init__(
        ...
        self.beta = beta
        self.loss_fn = DPOLoss(self.beta, self.args.label_smoothing, self.args.ipo, self.args.simpo)
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens):
        ...
        if self.args.simpo:
            all_logps = self._get_batch_logps(all_logits, input_ids, att_masks, prompt_id_lens, average_log_prob=True)
        else:
